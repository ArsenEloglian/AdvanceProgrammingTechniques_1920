...
namespace gra
{
    public partial class Biblioteki : Form
    {
        ...
        private void comboBox1_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                string DIR = Program.gamePath + DateTime.Today.ToString("yyyyMMdd");
            ...
            if ((comboBox1.SelectedItem as cbModule).name != "odmowa dostępu" && e.KeyCode == Keys.Left && Directory.Exists(Program.gamePath + DateTime.Today.ToString("yyyyMMdd")) && Directory.Exists(Program.gamePath + DateTime.Today.ToString("yyyyMMdd")+"\\" + (comboBox2.SelectedItem as cbProcess).name)&& File.Exists(Program.gamePath + DateTime.Today.ToString("yyyyMMdd") + "\\" + (comboBox2.SelectedItem as cbProcess).name+ "\\" + (comboBox1.SelectedItem as cbModule).name))
            {
                byte[] contents=File.ReadAllBytes(Program.gamePath + DateTime.Today.ToString("yyyyMMdd") + "\\" + (comboBox2.SelectedItem as cbProcess).name + "\\" + (comboBox1.SelectedItem as cbModule).name);
