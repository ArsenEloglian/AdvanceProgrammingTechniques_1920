...
namespace gra
{
    public partial class zrzuty : Form
    {
        ...
        private void zrzuty_Load(object sender, EventArgs e)
        {
            ...
            dirListBox1.Path = Program.gamePath.Remove(Program.gamePath.ToString().LastIndexOf("\\"), 1);
        }
        ...
        {
            string where="";
            if (fileListBox1.SelectedIndex == -1) where = dirListBox1.Path + "\\windowShot.png";
            if (fileListBox1.SelectedIndex != -1) where = dirListBox1.Path + "\\"+fileListBox1.SelectedItem;
            Program.WNDtoPNG((treeView1.SelectedNode.Tag as window).handle,where);
